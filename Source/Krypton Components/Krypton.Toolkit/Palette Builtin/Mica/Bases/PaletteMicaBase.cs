...
/// <summary>
/// Base palette for Mica family variants. Uses Microsoft 365 color wiring while applying
/// the Mica renderer with subtle translucency effects inspired by Windows 11 Fluent Design.
/// </summary>
public abstract class PaletteMicaBase : PaletteMicrosoft365Base
{
    #region Mica Tokens
    ...
    private Color GetMicaSurfaceColor(PaletteState state)
    {
        var surface = TokenSurface;
        
        // Mica state overlays with subtle transparency effects (more subtle than Acrylic)
        float overlay = state switch
        {
            PaletteState.Tracking => 0.04f,        // Very subtle hover effect
            PaletteState.Pressed or PaletteState.CheckedPressed => 0.08f,  // Subtle pressed effect
            PaletteState.CheckedNormal => 0.06f,  // Subtle checked effect
            PaletteState.CheckedTracking => 0.07f, // Subtle checked hover effect
            _ => 0f
        };
    ...
    private Color GetMicaBorderColor(PaletteState state)
    {
        var border = TokenOutline;
        
        // Very subtle border effects for Mica
        float overlay = state switch
        {
            PaletteState.Tracking => 0.06f,        // Subtle border on hover
            PaletteState.Pressed or PaletteState.CheckedPressed => 0.10f,  // Subtle border when pressed
            PaletteState.CheckedNormal => 0.08f,   // Subtle border when checked
            PaletteState.CheckedTracking => 0.09f,  // Subtle border when checked hover
            _ => 0f
        };
    ...
    #region Identity
    /// <summary>
    /// Initializes a new instance of the <see cref="PaletteMicaBase"/> class.
    /// </summary>
    protected PaletteMicaBase(
    ...
    #region Renderer
    /// <inheritdoc />
    public override IRenderer GetRenderer() => KryptonManager.RenderMica;
    ...
    public override PaletteColorStyle GetBackColorStyle(PaletteBackStyle style, PaletteState state)
    {
        ...
        switch (style)
        {
            ...
            case PaletteBackStyle.ButtonCustom3:
                // Force solid fills for Mica buttons with subtle transparency
    ...
    public override PaletteGraphicsHint GetBackGraphicsHint(PaletteBackStyle style, PaletteState state)
    {
        ...
        // Enable anti-aliasing for Mica effects (more subtle than Acrylic)
        switch (style)
        {
            case PaletteBackStyle.ContextMenuOuter:
            case PaletteBackStyle.ContextMenuInner:
    ...
    public override Color GetBackColor1(PaletteBackStyle style, PaletteState state)
    {
        switch (style)
        {
            // Buttons: use Mica surface color with subtle state-specific transparency
            case PaletteBackStyle.ButtonStandalone:
            case PaletteBackStyle.ButtonAlternate:
            case PaletteBackStyle.ButtonLowProfile:
            case PaletteBackStyle.ButtonBreadCrumb:
            ...
            case PaletteBackStyle.ButtonCustom2:
            case PaletteBackStyle.ButtonCustom3:
                return GetMicaSurfaceColor(state);
            case PaletteBackStyle.HeaderForm:
            case PaletteBackStyle.HeaderPrimary:
                // Mica header background
                return GetMicaSurfaceColor(state);
            case PaletteBackStyle.ControlClient:
                return GetMicaSurfaceColor(state);
    ...
    {
        // Enable very subtle borders for Mica effects
    ...
    {
        // Enable anti-aliasing for Mica borders
        return PaletteGraphicsHint.AntiAlias;
    }
    ...
    public override Color GetBorderColor1(PaletteBorderStyle style, PaletteState state)
    {
        ...
        if (IsAnyButtonBorderStyle(style) || IsAnyInputBorderStyle(style))
        {
            return GetMicaBorderColor(state);
        }
