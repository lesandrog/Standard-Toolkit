...
public abstract class PaletteMacOSBase : PaletteMaterialBase
{
    ...
    {
        if (state == PaletteState.NormalDefaultOverride && IsMacOSButtonBorderStyle(style))
        {
            // For macOS, the default button border color is the same as the normal border color, which is the accent blue color.
            return BaseColors?.ButtonNormalDefaultBorder ?? GetMacOSAccentBlueColor();
        }
    ...
    public override Color GetBackColor1(PaletteBackStyle style, PaletteState state)
    {
        ...
        if (!CommonHelper.IsOverrideState(state) && IsMacOSListSelectionState(state))
        {
            ...
            {
                return GetMacOSAccentBlueColor();
            }
        }
    ...
    private Color GetMacOSAccentBlueColor() =>
    ...
    private Color GetMacOSAccentBlueColor2() =>
        BaseColors?.ButtonNormalDefaultBack2 ?? GetMacOSAccentBlueColor();
    ...
    private static bool IsMacOSTabBackStyle(PaletteBackStyle style) =>
        style switch
        {
            PaletteBackStyle.TabHighProfile or PaletteBackStyle.TabStandardProfile or PaletteBackStyle.TabLowProfile
                ...
                or PaletteBackStyle.TabCustom1 or PaletteBackStyle.TabCustom2 or PaletteBackStyle.TabCustom3 => true,
            _ => false
        };
